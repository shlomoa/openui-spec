...
Sep 23, 2026 · <shlomo.anglister@gmail.com>
...
1. Commit the survey results under `survey/` (one file per framework, same columns).
2. Build `survey/matrix.csv`: concept × {W3C/HTML, WAI-ARIA, openui5, Qt, Angular Material, 4th}. Columns: name, category in that framework, key properties, events.
3. Flag each row: *same term/same meaning*, *same term/different meaning*, *different term/same meaning*, *unique*. *Validate:* script checks every catalog type appears in the matrix. *Demo:* sortable/filterable matrix web page.

**W1 Terminology**

1. Pick the canonical-term rule (e.g. W3C/ARIA name first, then majority across frameworks) and record it as a decision.
2. Extend the glossary to cover every catalog type and every matrix row flagged as a conflict.
3. Add the alias table (canonical term → openui5 / Qt / Angular Material / ARIA names). *Validate:* lint task 2.3 passes. *Demo:* searchable glossary page with alias lookup.

**W2 Scope**

 1. Write the normative scope section: purpose, audience, in scope, out of scope, deferred to later editions.
 2. Classify every catalog object and every survey concept as in / out / deferred. *Validate:* no catalog object is out of scope. *Demo:* scope map page.
 3. Split `docs/REQUIREMENTS.md` so spec requirements and generator requirements are separate.

**W3 UI categorization**

 1. Choose the primary axis (see open questions) and define the category set with inclusion rules.
 2. Re-map all 42 leaf scopes and all taxonomy entries to it; turn the other two taxonomies into informative views generated from the mapping.
 3. Rename or move scope folders to match. *Validate:* every object has exactly one primary category; catalog regenerates. *Demo:* interactive taxonomy tree with per-framework overlay.

**W4 Specification structure**

 1. Define the v1.0 outline, e.g.: 1 Introduction & scope · 2 Conformance · 3 Terminology · 4 Document model & language · 5 Categories & objects · 6 Catalog · Annex A Grammar · Annex B Survey mapping · Annex C Examples.
 2. Define RFC 2119 keyword use (MUST/SHOULD/MAY) and mark normative vs. informative sections.
 3. Move incremental-generation and generator content out of `spec/README.md` into the generator docs.

**W5 UI description language**

 1. Decide attribute value typing (string/null only vs. typed values).
 2. Replace the Angular-flavoured `[x]` / `(x)` key syntax with a framework-neutral one for Uses / Produces / Behaves, or formally adopt it.
 3. Define data-binding references, event payloads and i18n string references.
 4. Define versioning and compatibility policy (SemVer for the spec; how documents declare the version).
 5. Update `EBNF.txt` and `openui.schema.json` together. *Validate:* EBNF and JSON Schema accept/reject the same fixture set. *Demo:* live playground page — paste JSON, see validation and rendered tree.

**W6 Draft first spec**

 1. Rewrite the spec per W4 outline, using W1–W5 outputs.
 2. Enrich each leaf scope (Attributes, Child model) from the survey matrix; add evidence rows.
 3. Regenerate `openui.json`, bump to `1.0.0-rc.1`, migrate all examples and fixtures.
 4. Review period, then release `1.0.0`. *Validate:* full CI + conformance suite. *Demo:* published spec site with a rendered example per object (reuse `generated-examples`).

**W7 Documentation**

 1. Update README, REQUIREMENTS, CONTRIBUTING, RELEASING, AGENTS.md / CLAUDE.md / GEMINI.md, `.github/copilot-instructions.md`, agent files under `.github/agents/`.
 2. Write CHANGELOG `1.0.0` with a 0.2 → 1.0 migration guide.
 3. Publish to Read the Docs. *Demo:* the site itself.
 4. Notify downstream: angular-django2 (#98/#103 TS parser) and django-angular3.

**W8 Spec utilities**

 1. Create a shared conformance suite (`spec/conformance/`: valid + invalid documents with expected diagnostics).
 2. Python: parse (EBNF + JSON) → typed object model → validate (grammar, catalog membership, scope contract).
 3. TypeScript: same API surface in `@shlomoa/openui-spec`.
 4. Both packages pass the same suite; publish `1.0.0` to PyPI and npm. *Demo:* the W5 playground uses the TS validator.
