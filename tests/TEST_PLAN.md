...
| `test_table_family_contract.py`            | `TableContractTest`                    | The unified `Widgets/table.scope.md` leaf carries required sections and identity, documents behavior attributes (`(sort)`, `(filter)`, `(paginate)`), declares child model `tableRow`, and verifies `Controls/Table` is retired.                           |
